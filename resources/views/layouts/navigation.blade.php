<nav x-data="{ open: false }" class="bg-white fixed top-0 left-0 right-0 z-50 border-b border-gray-100">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 sm:gap-2">
                <x-lang-switch :availableLocales="config('app.available_locales', ['en' => 'English'])" :currentLocale="app()->getLocale()" />
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div class="flex items-center space-x-2 flex-col">
                                <div>{{ Auth::user()->first_name . ' ' . Auth::user()->second_name }}</div>
                                <span class="text-sm text-gray-600">{{ Auth::user()->getRoleNames()->first() }}</span>
                            </div>
