<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ $instructor->user->getTranslation('first_name','en')}}
        </h2>
    </x-slot>

    <div class="py-8 max-h-[calc(100vh-12rem)]">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="md:flex md:flex-col md:items-center text-center">
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-6">
                                    <h1 class="text-2xl font-bold text-gray-900">
                                        {{ $instructor->user->getTranslation('first_name',app()->getLocale()) }} {{ $instructor->user->getTranslation('second_name',app()->getLocale()) }}
                                    </h1>
                                    ...
                                    <div class="mt-1 flex flex-col sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-6">
                                        <div class="mt-2 flex items-center text-sm text-gray-500">
                                            ...
                                            {{ $instructor->experience ?? '0' }} {{ __('instructors.years_experience') }}
                                        </div>
                                        <div class="mt-2 flex items-center text-sm text-gray-500">
                                            <svg class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                <path d="M10.394 2.08a1 1 0 00-.788 0l-7 3a1 1 0 000 1.84l5.5 2.35L9.49 9.42a1 1 0 01.2-.2l1.8-1.8 5.5 2.35a1 1 0 00.94-.06l4-2.5a1 1 0 00.32-1.41l-8-10z" />
                            ...
                            <div class="mt-6 flex flex-wrap gap-3 justify-center">
                                ...
                                <form action="{{ route('instructors.destroy', $instructor) }}" method="POST" onsubmit="return confirm('{{ __('instructors.confirm_delete') }}')">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <div class="mt-6 flex flex-shrink-0 md:mt-0 md:ml-4">
                            @if($instructor->twitter_url)
                                <a href="{{ $instructor->twitter_url }}" target="_blank" class="text-gray-400 hover:text-blue-500">
                            ...
                            @if($instructor->linkedin_url)
                                <a href="{{ $instructor->linkedin_url }}" target="_blank" class="ml-3 text-gray-400 hover:text-blue-700">
                            ...
                            @if($instructor->facebook_url)
                                <a href="{{ $instructor->facebook_url }}" target="_blank" class="ml-3 text-gray-400 hover:text-blue-600">
                            ...
                            @if($instructor->youtube_url)
                                <a href="{{ $instructor->youtube_url }}" target="_blank" class="ml-3 text-gray-400 hover:text-red-600">
                                    <span class="sr-only">YouTube</span>
                                    <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                        <path fill-rule="evenodd" d="M19.812 5.418c.861.23 1.538.907 1.768 1.768.386 1.445.42 4.46.42 4.81s-.034 3.365-.42 4.81c-.23.861-.907 1.538-1.768 1.768-1.445.386-7.277.42-7.687.42s-6.242-.034-7.687-.42c-.861-.23-1.538-.907-1.768-1.768C2.034 15.365 2 12.35 2 12s.034-3.365.42-4.81c.23-.861.907-1.538 1.768-1.768C5.633 5.034 11.465 5 11.875 5s6.242.034 7.687.42zM10 15.5l6-3.5-6-3.5v7z" clip-rule="evenodd" />
                                    </svg>
                                </a>
                            @endif
                        </div>
            ...
            <div class="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-3">
                    ...
                    <div class="mt-8 bg-white shadow rounded-lg overflow-hidden">
                        ...
                        <div class="px-6 py-4">
                            <div class="space-y-4">
                                ...
                                <div>
                                    <h4 class="font-medium text-gray-900">{{ $instructor->company ?? __('instructors.no_company') }}</h4>
                                    <p class="text-sm text-gray-500">{{ $instructor->experience ?? '0' }} {{ __('instructors.years_experience') }}</p>
                ...
                <div class="space-y-8 mx-auto w-full">
                    ...
                    <div class="bg-white shadow rounded-lg overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-200">
                            <h3 class="text-lg font-medium text-gray-900">{{ __('instructors.contact_information') }}</h3>
                        </div>
                        <div class="px-6 py-4">
                            <ul class="space-y-3">
                    ...
                    <div class="bg-white shadow rounded-lg overflow-hidden">
                        ...
                        <div class="px-6 py-4">
                            <div class="space-y-2">
                                <h4 class="font-medium text-gray-900">{{ $instructor->education ?? __('instructors.no_education') }}</h4>
                                <p class="text-sm text-gray-600">{{ $instructor->experience ?? '0' }} {{ __('instructors.years_experience') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="w-full md:w-2/3 lg:w-3/4">
                            <h3 class="text-2xl font-bold text-gray-900 mb-6">{{ $instructor->user->first_name }} {{ $instructor->user->second_name }}</h3>
                                ...
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <dl class="space-y-3">
                                        ...
                                        @if($instructor->user->gender)
                                            <div class="sm:flex">
                                                <dt class="text-sm font-medium text-gray-500 sm:w-32 sm:flex-shrink-0">{{ __('instructors.gender') }}</dt>
                                                <dd class="text-sm text-gray-900">{{ ucfirst(__('instructors.' . $instructor->user->gender)) }}</dd>
                                            </div>
                                        @endif
                                        @if($instructor->user->birth_date)
                                            <div class="sm:flex">
                                                <dt class="text-sm font-medium text-gray-500 sm:w-32 sm:flex-shrink-0">{{ __('instructors.birth_date') }}</dt>
                                                <dd class="text-sm text-gray-900">{{ \Carbon\Carbon::parse($instructor->user->birth_date)->format('M d, Y') }}</dd>
                                            </div>
                                        @endif
                                    </dl>
                                </div>
