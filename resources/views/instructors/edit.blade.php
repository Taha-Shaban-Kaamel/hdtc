    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white max-h-[calc(100vh-12rem)] overflow-scroll shadow-sm sm:rounded-lg p-6">
                <x-form-section method="POST" action="{{ route('instructors.update', $instructor) }}">
                    @method('PUT')
                    ...
                    <x-slot name="description" class="text-gray-600">
                        {{ __('instructors.update_instructor_details') }}
                    </x-slot>
                    ...
                    <x-slot name="form" class="grid grid-cols-6 gap-6">
                        ...
                        <div class="col-span-6 flex flex-col items-center">
                            <div class="relative">
                                <img class="h-24 w-24 rounded-full object-cover" 
                                     src="{{ $instructor->profile_photo_url ?? 'https://ui-avatars.com/api/?name='.urlencode($instructor->name).'&color=7F9CF5&background=EBF4FF' }}" 
                                     alt="{{ $instructor->name }}">
                            </div>
                        </div>
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            ...
                            <x-input id="name" type="text" class="mt-1 block w-full" name="name" :value="old('name', $instructor->name)" required autofocus />
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            ...
                            <x-input id="email" type="email" class="mt-1 block w-full" name="email" :value="old('email', $instructor->email)" required />
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            ...
                            <x-input id="phone" type="tel" class="mt-1 block w-full" name="phone" :value="old('phone', $instructor->phone)" />
                        ...
                        <div class="col-span-6">
                            ...
                            <textarea id="bio" name="bio" rows="3" class="mt-1 block w-full border-gray-300 focus:border-[#066B87] focus:ring focus:ring-[#066B87] focus:ring-opacity-50 rounded-md shadow-sm">{{ old('bio', $instructor->bio) }}</textarea>
                            <x-input-error for="bio" class="mt-2" />
                        </div>

                        <!-- Profile Photo Update -->
                        <div class="col-span-6">
                            <x-label for="photo" value="{{ __('instructors.update_profile_photo') }}" />
                    ...
                    <x-slot name="actions">
                        <x-button type="submit" class="px-6 py-2.5 bg-[#066B87] hover:bg-[#055a72] text-white font-medium text-sm leading-tight rounded-md shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#066B87] transition duration-150 ease-in-out">
                            {{ __('instructors.update_instructor') }}
                        </x-button>
                    </x-slot>
                </x-form-section>
            </div>
